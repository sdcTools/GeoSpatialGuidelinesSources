...
\mdseries
{\large July 25, 2025}\vspace{36pt}
...
\include{chapters/1-General_concepts}

\chapter{Measuring Disclosure Risk for Geo-Referenced Data} 
\chaptermark{Disclosure Risk}
\label{sec:risk}
    \input{chapters/12a_sensitivity}
    \input{chapters/12-differencing_issues}

\include{chapters/10-Utility_metrics}

\chapter{Disclosure Control Methods for Geo-Referenced Data} \label{sec:methods}
\chaptermark{Methods}
    \input{chapters/16-quadtree}
    \input{chapters/13-SpatialSmoothing}
    \input{chapters/11-CKM_method}
    \input{chapters/15-TRS_method}
    \input{chapters/14-AuxiliaryMethods}
    \input{chapters/17-methods_summaries}

\chapter{Case Study: Population grids with the Cell Key Method} \label{sec:cs_ckm}
\chaptermark{Case Study}
    \input{chapters/11b-CKM_case_study}
